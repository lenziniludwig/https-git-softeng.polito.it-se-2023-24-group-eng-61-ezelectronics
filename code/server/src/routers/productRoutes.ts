...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("arrivalDate").isString().optional().isISO8601(),
        ...
        this.router.patch(
            ...
            body("quantity").isNumeric().isInt({ gt: 0 }),
            body("changeDate").isString().optional().isISO8601(),
            this.errorHandler.validateRequest.bind(this.errorHandler),
            this.authenticator.isLoggedIn.bind(this.authenticator),
            this.authenticator.isAdminOrManager.bind(this.authenticator),
        ...
        this.router.patch(
            ...
            param("model").isString().notEmpty(),
            body("quantity").isNumeric().isInt({ gt: 0 }),
            body("sellingDate").isString().optional().isISO8601(),
            this.errorHandler.validateRequest.bind(this.errorHandler),
            this.authenticator.isLoggedIn.bind(this.authenticator),
            this.authenticator.isAdminOrManager.bind(this.authenticator),
        ...
        this.router.get(
            "/",
            query("grouping").isString().optional().isIn([null, "category", "model"]),
            query("category").isString().optional().isIn(["Smartphone", "Laptop", "Appliance"]),
            query("model").isString().optional().notEmpty(),
            this.errorHandler.validatefilterRequest.bind(this.errorHandler),
            this.authenticator.isLoggedIn.bind(this.authenticator),
            this.authenticator.isAdminOrManager.bind(this.authenticator),
            (req: any, res: any, next: any) => this.controller.getProducts(req.query.grouping, req.query.category, req.query.model)
                .then((products: Product[] /*Product[]*/) => res.status(200).json(products))
                .catch((err) => next(err))
        )
