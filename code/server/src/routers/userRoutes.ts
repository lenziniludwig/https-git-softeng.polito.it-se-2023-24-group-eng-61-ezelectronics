...
class UserRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("role").notEmpty().isString().isIn(["Manager", "Customer", "Admin"]),
        ...
        this.router.get(
            "/",
            this.authService.isLoggedIn.bind(this.authService),
            this.authService.isAdmin.bind(this.authService),
            (req: any, res: any, next: any) => this.controller.getUsers(req.user)
                .then((users: User[] /**User[] */) => res.status(200).json(users))
                .catch((err) => next(err))
        )
        ...
        this.router.get(
            ...
            this.authService.isLoggedIn.bind(this.authService),
            this.authService.isAdmin.bind(this.authService),
            param("role").notEmpty().isString().isIn(["Manager", "Customer", "Admin"]),   
            this.errorHandler.validateRequest, 
            (req: any, res: any, next: any) => this.controller.getUsersByRole(req.user, req.params.role)
                .then((users: User[] /**User[] */) => res.status(200).json(users))
                .catch((err) => next(err))
        )
        ...
        this.router.get(
            ...
            (req: any, res: any, next: any) => this.controller.getUserByUsername(req.user, req.params.username)
                .then((user: User /**User */) => res.status(200).json(user))
                .catch((err) => next(err))
        )
        ...
        this.router.delete(
            ...
            this.authService.isLoggedIn.bind(this.authService),
            this.authService.isAdmin.bind(this.authService),
            (req: any, res: any, next: any) => this.controller.deleteAll(req.user)
                .then(() => res.status(200).end())
                .catch((err: any) => next(err))
        )
        ...
        this.router.patch(
            ...
            body("birthdate").notEmpty().isString().isISO8601(),
            this.authService.isLoggedIn.bind(this.authService),
            this.errorHandler.validateRequest,
...
class AuthRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("password").notEmpty().isString(),
            this.errorHandler.validateRequest,
            this.authService.isLoggedOut.bind(this.authService),
