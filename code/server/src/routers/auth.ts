...
import { Utility, InvalidCredentialsError, UserAlreadyLoggedInError } from "../utilities"
...
class Authenticator {
     ...
     */
    isLoggedOut(req: any, res: any, next: any) {
        if (!req.isAuthenticated()) return next()
        return res.status(401).json({ error: "User is already logged in", status: 401 })
    }
